...
// Per-session token: without it any local website could scan localhost ports
// and read vault files through this server. Regenerated on every launch.
const mediaToken = randomBytes(32).toString('hex')

// True only if `abs` resolves inside `root` (prefix startsWith is bypassable
// by sibling dirs like /vault-evil, and doesn't normalize `..`)
  ...
  return new Promise<void>((resolve) => {
    mediaServer = createServer((req, res) => {
      ...
      try {
        ...
        if (url.searchParams.get('token') !== mediaToken) {
...
// Board ids are UUIDs generated by this app — reject anything else before it
// reaches a filesystem path (blocks `../` traversal through the id argument)
function safeBoardId(id: unknown): string {
  if (typeof id !== 'string' || !/^[A-Za-z0-9-]{1,64}$/.test(id)) throw new Error('Invalid board id')
  return id
}

// Resolve a vault-relative path, throwing if it escapes the vault
function vaultContainedPath(rel: unknown): string {
  if (!vaultPath || typeof rel !== 'string') throw new Error('Invalid path')
  const abs = join(vaultPath, rel)
  if (!isInside(vaultPath, abs)) throw new Error('Path escapes vault')
  return abs
}
...
app.whenReady().then(async () => {
  ...
  // IPC to expose port + auth token synchronously to the preload
  ipcMain.on('media-server-info', (event) => {
    event.returnValue = { port: mediaServerPort, token: mediaToken }
  })
...
ipcMain.handle('files:open-external', (_e, relativePath: string) => {
  return shell.openPath(vaultContainedPath(relativePath))
})

ipcMain.handle('files:show-in-finder', (_e, relativePath: string) => {
  shell.showItemInFolder(vaultContainedPath(relativePath))
})

// ─── IPC — App ────────────────────────────────────────────────────────────────
ipcMain.handle('get-version', () => app.getVersion())
ipcMain.handle('update:check', () => autoUpdater.checkForUpdates())
// The install flow is automatic once an update is found; this re-triggers a
// check so the preload's installUpdate() doesn't reject with "no handler"
ipcMain.handle('update:install', () => autoUpdater.checkForUpdates())
